...
class ScheduleControllerTest {
    ...
    companion object {
        ...
        private const val SCHEDULES_API_PATH = "/api/calendars/{calendarId}/schedules"
        private const val SCHEDULE_API_PATH = "/api/calendars/{calendarId}/schedules/{scheduleId}"
    }

    private val tomorrow: LocalDateTime = LocalDateTime.now().plusHours(24)
    private lateinit var startTime1: LocalDateTime
    private lateinit var endTime1: LocalDateTime
    private lateinit var startTime2: LocalDateTime
    private lateinit var endTime2: LocalDateTime
    private lateinit var formattedStartTime1: String
    private lateinit var formattedEndTime1: String
    private lateinit var formattedStartTime2: String
    private lateinit var formattedEndTime2: String

    private var calendarId: Long = 1L
    private var scheduleId1: Long = 0L
    private var scheduleId2: Long = 0L
    private lateinit var username: String

    private lateinit var meetingScheduleDto: ScheduleRequestDto
    private lateinit var workoutScheduleDto: ScheduleRequestDto
    ...
    fun setUp() {
        ...
        // 테스트용 DTOs 생성
        meetingScheduleDto = ScheduleRequestDto(
            "회의 일정",
            "팀 회의",
            startTime1,
            endTime1,
            Location(37.5665, 126.9780, "서울특별시 중구 세종대로 110")
        )

        workoutScheduleDto = ScheduleRequestDto(
            "운동 일정",
            "헬스장 방문",
            startTime2,
            endTime2,
            Location(37.5678, 126.9890, "서울특별시 강남구 테헤란로 123")
        )
    }
    ...
    private fun createSchedule(dto: ScheduleRequestDto): Long {
        val requestBody = OBJECT_MAPPER.writeValueAsString(dto)

        val responseJson = testUserHelper.requestWithUserAuth(
            username,
            post(SCHEDULES_API_PATH, calendarId).content(requestBody)
        )
            .andDo(print())
    ...
    private fun assertScheduleResponse(
    ...
    fun t1() {
        ...
        // 첫 번째 일정 생성 테스트
        val requestBody1 = OBJECT_MAPPER.writeValueAsString(meetingScheduleDto)
        val resultActions1 = testUserHelper.requestWithUserAuth(
            username,
        ...
        // 두 번째 일정 생성 테스트
        val requestBody2 = OBJECT_MAPPER.writeValueAsString(workoutScheduleDto)
        val resultActions2 = testUserHelper.requestWithUserAuth(
            username,
            post(SCHEDULES_API_PATH, calendarId)
...
//    }
    ...
    fun t3() {
        // 특정 날짜의 일정 목록 조회
        val resultActions = testUserHelper.requestWithUserAuth(
            username,
    ...
    fun t6() {
        // 존재하지 않는 일정 ID 계산
        val lastScheduleId: Long = scheduleRepository.findTopByOrderByIdDesc()
            .map { it.id ?: 0L } // id가 null이면 0L을 반환
            .orElse(0L) // 최종적으로 Long 타입 유지

        val nonExistentScheduleId = lastScheduleId + 1
    ...
    fun t7() {
        // 수정할 일정 정보 준비
        val newStartTime = tomorrow.withHour(17).withMinute(0).withSecond(0).withNano(0)
        val newEndTime = newStartTime.plusHours(3)
        val formattedNewStartTime = newStartTime.format(FORMATTER)
        val formattedNewEndTime = newEndTime.format(FORMATTER)

        // 수정 요청 DTO 생성
        val updateDto = ScheduleRequestDto(
            "수정된 일정 제목",
            "수정된 일정 설명",
            newStartTime,
            newEndTime,
            Location(37.5678, 126.9890, "서울특별시 중구 세종대로 110")
        )

        val requestBody = OBJECT_MAPPER.writeValueAsString(updateDto)
...
//    val t9() throws Exception {
//        val calendarId = 1L;
//        val scheduleId = scheduleId1;
//
...
//        val resultActions = testUserHelper.requestWithUserAuth(username,
